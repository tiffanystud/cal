...
function defaultResp($message) {
    http_response_code(400);
    return ["error" => $message];
}
function getHandler() {
    $path = $_SERVER["PATH-INFO"];
    if ($path === "/users") {
        if (isset($_GET["id"])) {
            DBIO::getUserById($_GET["id"]);
        }
        return DBIO::getAllUsers();
    } else if ($path === "/groups") {
        if (isset($_GET["id"])) {
            return DBIO::getGroupById($_GET["id"]);
        }        
        return DBIO::getAllGroups();
    } else if ($path === "/users_groups") {
        return DBIO::getAllUsersGroups();
    } else {
        return defaultResp("Bad request");
    }
}
